...
public class PayrollService : IPayrollService
{
    ...
    private static byte[] BuildPayrollPeriodExcel(PayrollPeriodDetailDto detail, string companyName, string companyInfo)
    {
        ...
        sheet.Range(1, 1, 1, 13).Merge().Style.Font.SetBold().Font.SetFontSize(16);
        sheet.Cell(2, 1).Value = companyInfo;
        sheet.Range(2, 1, 2, 13).Merge().Style.Font.SetFontSize(10).Font.SetFontColor(XLColor.FromHtml("#51648F"));
        sheet.Cell(3, 1).Value = $"Period: {detail.Year}-{detail.Month:00} | {detail.StartDate:yyyy-MM-dd} to {detail.EndDate:yyyy-MM-dd}";
        sheet.Range(3, 1, 3, 13).Merge().Style.Font.SetFontSize(10).Font.SetFontColor(XLColor.FromHtml("#51648F"));
        sheet.Cell(4, 1).Value = $"Entries: {detail.Entries.Count:N0} | Total Net Payable: MVR {detail.TotalNetPayable:N2}";
        sheet.Range(4, 1, 4, 13).Merge().Style.Font.SetFontSize(10).Font.SetFontColor(XLColor.FromHtml("#51648F"));

        var headerRow = 6;
        sheet.Cell(headerRow, 1).Value = "Staff";
        sheet.Cell(headerRow, 2).Value = "Attended";
        sheet.Cell(headerRow, 3).Value = "Food Days";
        sheet.Cell(headerRow, 4).Value = "OT Pay";
        sheet.Cell(headerRow, 5).Value = "Salary Advance";
        sheet.Cell(headerRow, 6).Value = "Pension";
        sheet.Cell(headerRow, 7).Value = "Total Pay";
        sheet.Cell(headerRow, 8).Value = "Net Payable";
        sheet.Cell(headerRow, 9).Value = "Bank";
        sheet.Cell(headerRow, 10).Value = "Account Name";
        sheet.Cell(headerRow, 11).Value = "Account Number";
        sheet.Cell(headerRow, 12).Value = "Designation";
        sheet.Cell(headerRow, 13).Value = "Work Site";

        var headerRange = sheet.Range(headerRow, 1, headerRow, 13);
        ...
        {
            sheet.Cell(row, 1).Value = $"{entry.StaffCode} - {entry.StaffName}";
            sheet.Cell(row, 2).Value = entry.AttendedDays;
            sheet.Cell(row, 3).Value = entry.FoodAllowanceDays;
            sheet.Cell(row, 4).Value = entry.OvertimePay;
            sheet.Cell(row, 5).Value = entry.SalaryAdvanceDeduction;
            sheet.Cell(row, 6).Value = entry.PensionDeduction;
            sheet.Cell(row, 7).Value = entry.TotalPay;
            sheet.Cell(row, 8).Value = entry.NetPayable;
            sheet.Cell(row, 9).Value = entry.BankName ?? "-";
            sheet.Cell(row, 10).Value = entry.AccountName ?? "-";
            sheet.Cell(row, 11).Value = entry.AccountNumber ?? "-";
            sheet.Cell(row, 12).Value = entry.Designation ?? "-";
            sheet.Cell(row, 13).Value = entry.WorkSite ?? "-";
            row++;
        }

        var totalRow = row;
        sheet.Cell(totalRow, 1).Value = "TOTAL";
        sheet.Cell(totalRow, 8).Value = detail.TotalNetPayable;

        var totalRange = sheet.Range(totalRow, 1, totalRow, 13);
        ...
        {
            var dataRange = sheet.Range(headerRow + 1, 1, totalRow - 1, 13);
            dataRange.Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
            dataRange.Style.Border.InsideBorder = XLBorderStyleValues.Thin;
        }

        sheet.Range(headerRow + 1, 4, totalRow, 8).Style.NumberFormat.Format = "#,##0.00";
        sheet.Range(headerRow + 1, 2, totalRow, 3).Style.NumberFormat.Format = "#,##0";
        sheet.SheetView.FreezeRows(headerRow);
        sheet.Range(headerRow, 1, Math.Max(totalRow - 1, headerRow), 13).SetAutoFilter();
